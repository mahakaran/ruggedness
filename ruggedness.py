...
class NMaximaEstimator(RuggednessBase): 
    ...
    def get_nmaxima(self, incomplete=False, multiprocessing=False, num_workers=6):
        ...
        if multiprocessing: 
            ...
            if __name__ == '__main__':
                ...
                result  = pool.starmap(NMaximaEstimator._is_maximum, tqdm(grid))
                self.n_maxima = result
                self.n_lone_nodes = 'Multiprocessing does not support lone node calculation yet'
        else: 
            n    = 0
            lone = 0
            for entry in tqdm(self.landscape): 
...
class RandomWalkEstimator(RuggednessBase):
    ...
    def get_rw_statistics(self, n_steps=50, n_walks=100, epsilon=0.1, incomplete=False,
            ...
            for i in range(orders-1):  
                ...
                for seq in sequences:                                                                           # iterate over sequences
                    ...
                    for inter in ord_interact[order]:                                                           # iterate over all possible non-redundant interactions for this order (the basis expansion in positions)
                        vec_encode = EpistasisEstimator._vector_encoding(sequence=seq, order=order, encoding=encodings[order], positions=inter) # calculate one-hot vector for this sequence at a given interaction
...
class RSEstimator(EpistasisEstimator):
    ...
    def get_rs_ratio(self, model, available_memory=5, **model_kwargs):
...
class FourierEstimator(EpistasisEstimator): 
    ...
    def get_fourier_basis(self, model, available_memory=5, **model_kwargs): 
